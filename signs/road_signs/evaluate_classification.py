...
def evaluate_mapillary_class(weights: str):
    ...
    model.load_state_dict(torch.load(f'weights/{weights}', map_location=torch.device(device_type)))
    f = open(f'results_{weights}.txt', 'w')
...
def evaluate_unknown_class(weights: str):
...
def evaluate_german_class(weights: str):
